"""Shared normalization utilities used by the released evaluation code.
...
clipped to [0, 4500]. The final functions provide the sRGB target and three-band
SAR hint conversions used by the ControlNet reproduction.
"""
import numpy as np
...
def to_hwc(arr):
    """Convert (H,W,C) or (C,H,W) to (H,W,C)."""
    a = np.asarray(arr)
    if a.ndim == 2:
        return a[..., None]
    if a.ndim == 3:
        c_axis = int(np.argmin(a.shape))
        return np.moveaxis(a, c_axis, -1)
    raise ValueError(f"unexpected array shape: {a.shape}")
...
# ============================================================
#  ControlNet-specific sRGB target and hint transforms.
# ============================================================
...
def opt_to_srgb_m1p1(arr):
    """
    Convert a uint16 reflectance target to sRGB-normalized [-1,1] values.

    Returns an (H,W,3) float32 array for the ControlNet image pipeline.
    """
    a = to_hwc(arr).astype(np.float32)
    lo, hi = OPT_CLIP
    lin = np.clip((a - lo) / (hi - lo), 0.0, 1.0)
    srgb = _srgb_encode(lin)
    return (srgb * 2.0 - 1.0).astype(np.float32)


def srgb_m1p1_to_opt(x):
    """
    Convert a ControlNet VAE output in sRGB-normalized [-1,1] to reflectance.

    ``x`` may be NumPy or torch in CHW, HWC, or batched layout. Returns an
    (H,W,3) uint16 array.
    """
    a = np.asarray(x.detach().cpu()) if hasattr(x, 'detach') else np.asarray(x)
    if a.ndim == 4:
        a = a[0]
    a = to_hwc(a).astype(np.float32)
    s = np.clip((a + 1.0) / 2.0, 0.0, 1.0)
    lin = _srgb_decode(s)
    lo, hi = OPT_CLIP
...
def normalize_sar_hint01(arr):
    """
    Convert two-channel SAR to a three-channel ControlNet hint in [0,1].

    VV and VH are clipped independently; the third channel is zero-filled.
    ...
    for c, (lo, hi) in enumerate(SAR_CLIP):
        chans.append((np.clip(a[c], lo, hi) - lo) / (hi - lo))   # [0,1]
    zero = np.zeros_like(chans[0])
    hint = np.stack([chans[0], chans[1], zero], axis=0)          # (3,H,W)
    return np.transpose(hint, (1, 2, 0)).astype(np.float32)      # (H,W,3)
